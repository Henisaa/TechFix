...
export const userApi = axios.create({
  baseURL: import.meta.env.VITE_USER_SERVICE || 'http://localhost:8080/api/v1/users'
});
...
const setupInterceptors = (apiClient) => {
  apiClient.interceptors.response.use(
    ...
    (error) => {
      if (!error.response) {
        toast.error("Servicio temporalmente no disponible — por favor intenta más tarde");
