  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-xl shadow-md border border-amber-200">
      ...
      <p className="text-slate-600 text-center mb-6 max-w-md">
        Este módulo estará disponible cuando el microservicio correspondiente
        esté activo.
      </p>
      ...
      <div className="bg-slate-50 w-full rounded-lg p-4 border border-slate-200">
        ...
        <div>
          ...
          <ul className="space-y-2 font-mono text-sm">
            {plannedEndpoints &&
              plannedEndpoints.map((endpoint, index) => {
                ...
                if (method === "PATCH")
                  methodColor = "text-purple-600 font-bold";

                return (
                  <li
                    key={index}
                    className="flex gap-2 p-2 bg-white rounded border border-slate-100"
                  >
